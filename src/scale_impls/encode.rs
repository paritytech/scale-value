...
fn encode_variant_value<T: Clone>(
	...
	let variant_type = match variant_type {
		None => return Err(EncodeError::VariantNotFound { actual: variant.clone(), expected: type_id }),
		Some(v) => v,
	};
		...
		ValueDef::Composite(Composite::Unnamed(vals)) => {
			...
			for val in vals {
				match val.value {
					...
					_ => return Err(EncodeError::WrongShape { actual: val.clone(), expected: type_id }),
				}
			}
			bools
		}
