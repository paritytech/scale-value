...
mod test {
    ...
    fn assert_from_to<T: std::fmt::Debug + PartialEq>(val: Value<T>) {
        ...
        match crate::stringify::from_str(&s) {
            ...
            (Ok(new_val),rest) => {
                assert_eq!(val.remove_context(), new_val, "value should be the same after parsing to/from a string");
    ...
    #[test]
    fn variants() {
        assert_from_to(Value::named_variant("A weird variant name", vec![
            ("foo".into(), Value::uint(12345u128)),
            ("bar".into(), Value::bool(true)),
            ("a \"weird\" name".into(), Value::string("Woop!")),
        ]));
        assert_from_to(Value::unnamed_variant("MyVariant", vec![
            Value::uint(12345u128),
            Value::bool(true),
            Value::string("Woop!"),
        ]));
