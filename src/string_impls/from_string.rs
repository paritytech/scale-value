...
pub enum ParseStringError {
    #[error("Expected a closing quote to match the opening quote at position {0}")]
    ExpectedClosingQuoteToMatch(usize),
    #[error("The char '{0}' cannot be escaped")]
    CannotEscapeChar(char)
}
...
fn parse_named_composite(t: &mut impl Tokens<Item = char>) -> Result<Composite<()>, Option<ParseError>> {
    ...
    let vals = t.sep_by_err(
        |t| parse_field_name_and_value(t),
        |t| skip_spaced_separator(t, ',')
    ).collect::<Result<_,_>>()?;

    skip_whitespace(t);
    if !t.token('}') {
        return Err(Some(ParseComplexError::ExpectedCloserToMatch('}', start).at(t.offset())))
    }
    Ok(Composite::Named(vals))
}

// Parse an unnamed composite value like `(true, 123)`
fn parse_unnamed_composite(t: &mut impl Tokens<Item = char>) -> Result<Composite<()>, Option<ParseError>> {
    let start = t.offset();
    if !t.token('(') {
        return Err(None)
    }
...
fn parse_number(t: &mut impl Tokens<Item = char>) -> Result<Primitive, Option<ParseError>> {
    ...
    let is_positive = if t.token('+') || !t.token('-') {
        true
    } else {
        false
    };
...
fn parse_string(t: &mut impl Tokens<Item = char>) -> Result<String, Option<ParseError>> {
    ...
    loop {
        ...
        let char = match t.next() {
            Some(c) => c,
            None => return Err(Some(ParseStringError::ExpectedClosingQuoteToMatch(start).at(t.offset())))
        };
...
fn parse_field_name_and_value(t: &mut impl Tokens<Item = char>) -> Result<(String, Value<()>), ParseError> {
    let name = parse_field_name(t)?;
    if !skip_spaced_separator(t, ':') {
        return Err(ParseComplexError::MissingFieldSeparator(':').at(t.offset()))
    }
...
fn parse_ident(t: &mut impl Tokens<Item = char>) -> Result<String, ParseError> {
    ...
    if t.skip_tokens_while(|c| c.is_alphabetic()) == 0 {
        return Err(ParseComplexError::InvalidStartingCharacterInIdent.at(start.offset()))
    }
...
mod test {
    ...
    fn parse_variants() {
        assert_eq!(from_str("MyVariant {
            ...
            foo: 1234,
            \"Hello there 😀\": \"Hello!\"
        }"), Ok(Value::named_variant("MyVariant", vec![
            ("hello".into(), Value::bool(true)),
            ("foo".into(), Value::uint(1234u128)),
            ("Hello there 😀".into(), Value::string("Hello!"))
        ])));
